                ...
                if 'TimelineAddEntries' == info['type']:
                    for following_info in info['entries']:
                        if 'TimelineTimelineItem' == following_info['content']['entryType']:
                            ...
                            self.insert_or_update(table_name='twitter_user_detail', data_list=d)  # 保存用户信息
                            data.append(d)
    ...
    def execute(self):
        ...
        if data:
            ...
            for user_id in data:
                ...
                depth = self.seed.get('depth', 2)  # 深度
                d = dict(
                    connect=connect,
